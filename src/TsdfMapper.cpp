...
namespace voxblox
{
    TsdfMapper::TsdfMapper(common::Config::VoxbloxCfg& config){
        ...
        tsdf_integrator_ = TsdfIntegratorFactory::create(config.tsdf_methods, integrator_config, tsdf_map_->getTsdfLayerPtr());
...
namespace dynablox
{
    EverFreeIntegrator::EverFreeIntegrator(const common::Config::EverFreeCfg& config, TsdfLayer::Ptr tsdf_layer)
        : config_(config), tsdf_layer_(std::move(tsdf_layer)), 
