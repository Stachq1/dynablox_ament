...
namespace dynablox {
Clustering::Clustering(const common::Config::ClusterCfg& config, TsdfLayer::Ptr tsdf_layer)
    : config_(config), tsdf_layer_(std::move(tsdf_layer)), neighborhood_search_(config.neighbor_connectivity) {}
...
Clusters Clustering::performClustering(const BlockToPointMap& point_map,
                                       ...
                                       const Cloud& cloud, CloudInfo& cloud_info) const {
...
void Clustering::setClusterLevelDynamicFlagOfallPoints(const Clusters& clusters, CloudInfo& cloud_info) const {
...
bool Clustering::filterCluster(const Cluster& cluster) const {
  ...
  if (cluster_size < config_.min_cluster_size || cluster_size > config_.max_cluster_size) {
    return true;
  }
...
void Clustering::applyClusterLevelFilters(Clusters& candidates) const {
  candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
                                  [this](const Cluster& cluster) { return filterCluster(cluster); }),
                   candidates.end());
}
void Clustering::mergeClusters(const Cloud& cloud, Clusters& clusters) const {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (!first_cluster.aabb.intersects(second_cluster.aabb, config_.min_cluster_separation)) {
        second_id++;
      ...
      if (config_.check_cluster_separation_exact) {
        ...
        for (const int point_1 : first_cluster.points) {
          for (const int point_2 : second_cluster.points) {
            const float distance = (cloud[point_1].getVector3fMap() - cloud[point_2].getVector3fMap()).norm();
        ...
        for (const Point& point_1 : first_cluster.voxels) {
          for (const Point& point_2 : second_cluster.voxels) {
            const float distance = (point_1.getVector3fMap() - point_2.getVector3fMap()).norm();
            if (distance <= config_.min_cluster_separation) {
              distance_met = true;
              break;
            }
      ...
      if (distance_met) {
        first_cluster.points.insert(first_cluster.points.end(), second_cluster.points.begin(),
...
void Clustering::computeAABB(const Cloud& cloud, Cluster& cluster) const {
  ...
  PointType& min = cluster.aabb.min_corner;
  PointType& max = cluster.aabb.max_corner;
  if (config_.check_cluster_separation_exact) {
    ...
    for (size_t i = 1; i < cluster.points.size(); ++i) {
      const PointType& point = cloud[cluster.points[i]];
    ...
    min = PointType(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                    std::numeric_limits<float>::max());
    max = PointType(std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(),
                    std::numeric_limits<float>::lowest());
    for (const PointType& point : cluster.voxels) {
      min.x = std::min(min.x, point.x);
      min.y = std::min(min.y, point.y);
      min.z = std::min(min.z, point.z);
      max.x = std::max(max.x, point.x);
...
Clusters Clustering::inducePointClusters(const BlockToPointMap& point_map,
  ...
  for (const auto& voxel_cluster : voxel_cluster_indices) {
    ...
    for (const voxblox::VoxelKey& voxel_key : voxel_cluster) {
      ...
      const voxblox::Point center = voxblox::getCenterPointFromGridIndex(
          voxblox::getGlobalVoxelIndexFromBlockAndVoxelIndex(voxel_key.first, voxel_key.second, voxels_per_side),
          voxel_size);
      candidate_cluster.voxels.push_back(PointType(center.x(), center.y(), center.z()));
...
std::vector<Clustering::ClusterIndices> Clustering::voxelClustering(
    const ClusterIndices& occupied_ever_free_voxel_indices, const int frame_counter) const {
...
bool Clustering::growCluster(const voxblox::VoxelKey& seed, const int frame_counter, ClusterIndices& result) const {
  ...
  while (!stack.empty()) {
    ...
    const voxblox::AlignedVector<voxblox::VoxelKey> neighbors =
        neighborhood_search_.search(voxel_key.first, voxel_key.second, voxels_per_side);

    for (const voxblox::VoxelKey& neighbor_key : neighbors) {
      TsdfBlock::Ptr neighbor_block = tsdf_layer_->getBlockPtrByIndex(neighbor_key.first);
      if (!neighbor_block) {
        continue;
      }
      TsdfVoxel& neighbor_voxel = neighbor_block->getVoxelByVoxelIndex(neighbor_key.second);

      // If neighbor is valid add it to the cluster, and potentially keep
      // growing if it is ever-free.
      if (!neighbor_voxel.clustering_processed && neighbor_voxel.last_lidar_occupied == frame_counter) {
        if (neighbor_voxel.ever_free || (tsdf_voxel.ever_free && config_.grow_clusters_twice)) {
